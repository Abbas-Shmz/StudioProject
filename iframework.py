...
class Mode(Base):
    'personal, because in a groupPersons (family), some might have a scooter or rollers'
    __tablename__ = 'modes'
    ...
    startTime = Column(DateTime) # None first time if only one groupPersons
...
class Person(Base):
    ...
    #groupPersons = relationship('Group', backref = backref('persons'))
    ...
    def getGroups(self):
        if len(self.groupBelongings) > 0:
            return [gb.groupPersons for gb in self.groupBelongings]
        else:
            return None
...
class AbstractPassing:
    def initPersonGroupPassing(self, group, person, transport, vehicle):
        if person is None and group is not None: # create groupPersons
            self.group = group
            ...
            print('Warning: passing person and groupPersons or both None')
...
class ZonePassing(AbstractPassing,Base):
    __tablename__ = 'zonepassings'
    idx = Column(Integer, primary_key=True)
...
if __name__ == '__main__': # demo code
    ...
    counts.append(ZonePassing(zone, datetime(2020,7,7,9,5), True, person = Person(33, 'male', False, False, True, False)))
